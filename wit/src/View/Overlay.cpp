#include "Overlay.h"
#include "VTK/vtkOpenGLImageOverlayActor.h"
...
Overlay::Overlay(vtkRenderer *renderer, DTIScalarVolume *vol, float offset, float opacity, int cmi)
{
	_renderer = renderer;
	_visible = false;
	_opacity = opacity;
...
void Overlay::setupColorMap()
{
	...
	for_ij
	{
		...
		// TODO: Port from quench (we don't use a color map panel):
...
void Overlay::SetPosition(double *wPos, double *tPos)
{
	...
	double wDim[3];
	double wZero[3];
	WITVolumeViz::WorldToLocal(_mat,wPos,_dim, v);
	WITVolumeViz::LocalToWorld(_mat,lDim,wDim);
	WITVolumeViz::LocalToWorld(_mat,lZero,wZero);
	float v1[3] = {v[0], v[1], v[2]};
	//for(int i=0; i<3; i++) { flPos[i]/=(double)_vox_sz[i]; }
	
	double flPos[3];
	WITVolumeViz::WorldToLocal(_mat,wPos,_dim,flPos);
