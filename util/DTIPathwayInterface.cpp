...
{
  if(_point_vector.size()>=position) {
  ...
  if(_point_stat_array.size() >= position)
    _point_stat_array.erase(_point_stat_array.begin()+position);
  if(_path_stat_vector.size() >= position)
...
DTIPathwayInterface::getPointStatistic(PointProperty pointIndex, PathwayProperty statIndex)
{
  ...
  if (pointIndex >= _point_stat_array[statIndex].size()) {
    return 0.0;
  }
...
{
  if (statIndex < _path_stat_vector.size()) {
...
DTIPathwayInterface::insertBefore(const DTIVector &vec, int position)
{
  ...
  DTIGeometryVector *copy = new DTIGeometryVector(3,v);
  int curPos = 0;
...
DTIPathwayInterface::deepCopy(DTIPathwayFactoryInterface *pathFactory)
{
  ...
  for (int i = 0; i < _point_vector.size(); i++) {
...
DTIPathwayInterface::~DTIPathwayInterface()
{
  ...
  _path_stat_vector.clear();
  int k;
  for(k=0; k<_point_vector.size(); k++) {
...

{
  ...
  static DTIVector *p1 = new DTIVector(3, p1Data);
  static DTIVector *p2 = new DTIVector(3, p2Data);
  ...
  while (arcLengthRemaining > distance) {
    ...
    if (minIndex+2 >= pathway->getNumPoints()) {
      ...
      cerr << "path length: " << DTIStats::computeLength(pathway,-1) << endl;
...
{
  DTIPathwayInterface *newPathway = pathFactory->createPathway(_algo);
  double pathLength = DTIStats::computeLength (this, -1);
